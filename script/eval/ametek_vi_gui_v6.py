...
class PlotApp:
    ...
    def create_widgets(self):
        ...
        self.plot_type_combobox = ttk.Combobox(self.root, values=[
            "Voltage", "Current", "Voltage & Current", 
            "|Vac|", "|Iac|", "|Vac| & |Iac|",
            "Vacre", "Vacim", "Phase of Vac",
            "Iacre", "Iacim", "Phase of Iac",
            "Voltage & Vacre", "Current & Iacre"
        ])
    ...
    def plot_data(self):
        ...
        if plot_type in ["Voltage & Current", "|Vac| & |Iac|", "Voltage & Vacre", "Current & Iacre"]:
            ...
            if plot_type == "Voltage & Current":
                line1, = ax1.plot(combined_data['Elapsed Time (s)'], combined_data['Potential (V)'],
                                  label="Voltage", color=TU_COLORS['TU-Rot'])
                line2, = ax2.plot(combined_data['Elapsed Time (s)'], combined_data['Current (A)'],
            ...
            elif plot_type == "|Vac| & |Iac|":
                line1, = ax1.plot(combined_data['Elapsed Time (s)'], combined_data['|Vac| (V)'],
                                  label="|Vac|", color=TU_COLORS['TU-Rot'])
                line2, = ax2.plot(combined_data['Elapsed Time (s)'], combined_data['|Iac| (A)'],
                                  label="|Iac|", color=TU_COLORS['Blau'], linestyle='--')
                ax1.set_ylabel("|Vac| (V)", color=TU_COLORS['TU-Rot'])
                ax2.set_ylabel("|Iac| (A)", color=TU_COLORS['Blau'])
            elif plot_type == "Voltage & Vacre":
                line1, = ax1.plot(combined_data['Elapsed Time (s)'], combined_data['Potential (V)'],
                                  label="Voltage", color=TU_COLORS['TU-Rot'])
                line2, = ax2.plot(combined_data['Elapsed Time (s)'], combined_data['Vacre (V)'],
                                  label="Vacre", color=TU_COLORS['Orange'], linestyle='--')
                ax1.set_ylabel("Voltage (V)", color=TU_COLORS['TU-Rot'])
                ax2.set_ylabel("Vacre (V)", color=TU_COLORS['Orange'])
            else:  # Current & Iacre
                line1, = ax1.plot(combined_data['Elapsed Time (s)'], combined_data['Current (A)'],
                                  label="Current", color=TU_COLORS['Blau'])
                line2, = ax2.plot(combined_data['Elapsed Time (s)'], combined_data['Iacre (A)'],
                                  label="Iacre", color=TU_COLORS['Dunkelgrau'], linestyle='--')
                ax1.set_ylabel("Current (A)", color=TU_COLORS['Blau'])
                ax2.set_ylabel("Iacre (A)", color=TU_COLORS['Dunkelgrau'])
            ...
            if self.legend_var.get():
                ...
                ax1.legend(lines, labels, loc='best', fontsize=int(self.fontsize_legend_spinbox.get()))
            ...
            plot_config = {
                "Voltage": ("Potential (V)", TU_COLORS['TU-Rot']),
                "Current": ("Current (A)", TU_COLORS['Blau']),
                "|Vac|": ("|Vac| (V)", TU_COLORS['TU-Rot']),
                "|Iac|": ("|Iac| (A)", TU_COLORS['Blau']),
                "Vacre": ("Vacre (V)", TU_COLORS['Orange']),
                "Vacim": ("Vacim (V)", TU_COLORS['Violett']),
                "Phase of Vac": ("Phase of Vac (deg)", TU_COLORS['Grün']),
                "Iacre": ("Iacre (A)", TU_COLORS['Dunkelgrau']),
                "Iacim": ("Iacim (A)", TU_COLORS['Hellgrau']),
                "Phase of Iac": ("Phase of Iac (deg)", TU_COLORS['Schwarz'])
            }

            y_column, color = plot_config.get(plot_type, (plot_type, TU_COLORS['TU-Rot']))

            ax.plot(combined_data['Elapsed Time (s)'], combined_data[y_column],
                    label=plot_type, linestyle='-', linewidth=1, color=color)
            ax.set_ylabel(y_column)

            title = self.title_entry.get() if self.title_entry.get() else self.file_path.split('/')[-1]
            xlabel = self.xlabel_entry.get() if self.xlabel_entry.get() else 'Elapsed Time (s)'
            ylabel = self.ylabel_entry.get() if self.ylabel_entry.get() else y_column
