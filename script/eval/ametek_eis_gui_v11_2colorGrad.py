# -*- coding: utf-8 -*-
"""
Created on Sun Jul 28 15:05:27 2024
...
class PlotApp:
        ...
        if self.color_chosen:
            ...
            for i, segment in enumerate(selected_segments):
                ...
                line, = ax.plot(segment_data['Zre (ohms)'], segment_data['Zim (ohms)'],
                                label=legend_text, linestyle=line_style, linewidth=1, color=line_color, visible=False)
                lines.append(line)
                line_labels.append(legend_text)
    
                scatter = ax.scatter(segment_data['Zre (ohms)'], segment_data['Zim (ohms)'],
                                     marker='o', s=20, color=line_color, visible=False)
                ...
                red_dot = ax.scatter(specific_data['Zre (ohms)'], specific_data['Zim (ohms)'],
                                     marker='o', s=20, color='red', visible=False)
            ...
            for i, segment in enumerate(selected_segments):
                ...
                legend_text = legend_text.get() if legend_text and legend_text.get() else f'Segment {segment}'
                line, = ax.plot(segment_data['Zre (ohms)'], segment_data['Zim (ohms)'],
                                label=legend_text, linestyle=line_styles[i % len(line_styles)], linewidth=1, visible=False)
                lines.append(line)
                line_labels.append(legend_text)
                scatter = ax.scatter(segment_data['Zre (ohms)'], segment_data['Zim (ohms)'],
                                     marker=markers[i % len(markers)], s=20, visible=False)
                scatters.append(scatter)
                specific_data = segment_data[segment_data['Frequency (Hz)'].apply(
                    lambda x: any(10**j - 0.1*10**j <= x <= 10**j + 0.1*10**j for j in range(-6, 7)))]
                red_dot = ax.scatter(specific_data['Zre (ohms)'], specific_data['Zim (ohms)'],
                                     marker=markers[i % len(markers)], s=20, color='red', visible=False)
                red_dots.append(red_dot)
    ...
    def update_legend(self):
        if self.legend_var.get():
            handles, labels = [], []
            for line, label in zip(self.lines, self.line_labels):
                if line.get_visible():
                    handles.append(line)
                    labels.append(label)
            if handles:
