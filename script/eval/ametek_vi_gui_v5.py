# -*- coding: utf-8 -*-
"""
Created on Tue Oct  8 20:33:51 2024
...
class PlotApp:
    ...
    def create_widgets(self):
        ...
        self.plot_type_combobox = ttk.Combobox(self.root, values=["Voltage", "Current", "Voltage & Current", "|Vac|", "|Iac|", "|Vac| & |Iac|"])
    ...
    def plot_data(self):
        ...
        if plot_type in ["Voltage & Current", "|Vac| & |Iac|"]:
            ...
            if plot_type == "Voltage & Current":
                ...
                voltage_line, = ax1.plot(combined_data['Elapsed Time (s)'], combined_data['|Vac| (V)'],
                                         label="|Vac|", color=TU_COLORS['TU-Rot'])
                current_line, = ax2.plot(combined_data['Elapsed Time (s)'], combined_data['|Iac| (A)'],
            ...
            if plot_type == "Voltage":
                ax.plot(combined_data['Elapsed Time (s)'], combined_data['Potential (V)'],
                        label="Voltage", linestyle='-', linewidth=1, color=TU_COLORS['TU-Rot'])
                ax.set_ylabel("Potential (V)")
            elif plot_type == "Current":
                ax.plot(combined_data['Elapsed Time (s)'], combined_data['Current (A)'],
                        label="Current", linestyle='-', linewidth=1, color=TU_COLORS['Blau'])
                ax.set_ylabel("Current (A)")
            elif plot_type == "|Vac|":
                ax.plot(combined_data['Elapsed Time (s)'], combined_data['|Vac| (V)'],
                        label="|Vac|", linestyle='-', linewidth=1, color=TU_COLORS['TU-Rot'])
                ax.set_ylabel("|Vac| (V)")
            elif plot_type == "|Iac|":
                ax.plot(combined_data['Elapsed Time (s)'], combined_data['|Iac| (A)'],
                        label="|Iac|", linestyle='-', linewidth=1, color=TU_COLORS['Blau'])
                ax.set_ylabel("|Iac| (A)")

            title = self.title_entry.get() if self.title_entry.get() else self.file_path.split('/')[-1]
            xlabel = self.xlabel_entry.get() if self.xlabel_entry.get() else 'Elapsed Time (s)'
            ylabel = self.ylabel_entry.get() if self.ylabel_entry.get() else ax.get_ylabel()
