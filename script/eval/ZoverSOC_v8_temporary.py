...
@author: Jaehyun
Updated on Fri Aug 23 2024 to include Phase of Z plotting option, figure dimension control,
...
class ZoverSOCPlotter:
    def __init__(self):
        self.root = Tk()
        self.root.title("ZoverSOC Plotter")
        self.df = None
        self.show_legend = True
        self.use_simple_legend = False
        self.plot_type = StringVar(value="|Z| (ohms)")
        self.figure_width = StringVar(value="12")
        self.figure_height = StringVar(value="8")
        self.setup_gui()
    ...
    def setup_gui(self):
        ...
        self.show_legend_checkbox.select()
        ...
        export_button = Button(self.root, text="Export Results", command=self.export_results)
        export_button.pack()
    ...
    def export_results(self):
        if self.df is None:
            print("No data loaded. Please load data before exporting.")
            return
        ...
        export_df = self.df[['Frequency (Hz)', 'SOC', 'direction', '|Z| (ohms)', 'Phase of Z (deg)']]
        export_df = export_df[export_df['Frequency (Hz)'] != 0].sort_values(['Frequency (Hz)', 'SOC'])
